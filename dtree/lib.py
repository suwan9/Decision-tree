...
def get_entropy(fc2_s, fc2_s2, domain_temperature=1.0, class_temperature=10.0):# fc2_s3, fc2_s4, fc2_s5,
    ...
    entropy = -torch.sum(fc2_s * torch.log(fc2_s), 1).data.cpu().numpy()
    entropy2 = -torch.sum(fc2_s2 * torch.log(fc2_s2), 1).data.cpu().numpy()
    ...
    #print(entropy[0:15],entropy2[0:15],entropy3[0:15],entropy4[0:15],entropy5[0:15],'entropy')

    entropy_norm = np.log(fc2_s.size(1))
    #print(entropy_norm,'entropy_norm')

    #weight = (entropy + entropy2 + entropy3 + entropy4 + entropy5) / (5 * entropy_norm)
    #weight = (entropy + entropy2) / (2 * entropy_norm)
    weight = (entropy + entropy2) / 2
    return weight
...
def get_consistency(fc2_s, fc2_s2):#, fc2_s3, fc2_s4, fc2_s5
    ...
    c = torch.cat((fc2_s, fc2_s2), dim=1)
    d = torch.std(c, 1)
